    ...
    while (!frontier.empty()) {
      ...
      if (visited.find(node) == visited.end()) {
        ...
        if (node->left == nullptr && node->right == nullptr) {
          ...
          std::transform(std::begin(ancestors), std::end(ancestors),
                         std::begin(path)++,
                         [](auto const* a) { return a->val; });
          path.back() = node->val;
          if (sum == std::accumulate(std::begin(path), std::end(path), 0)) {
            paths.push_back(path);
          }
          while (!ancestors.empty()) {
            auto* b = ancestors.back();
            if ((b->left != nullptr &&
                 visited.find(b->left) == visited.end()) ||
                (b->right != nullptr &&
                 visited.find(b->right) == visited.end())) {
              break;
            }
            ancestors.pop_back();
          }
