...
struct Graph {
  ...
  auto size() -> size_t { return adj.size(); }
  struct PathAnalysis {
    bool has_cycle;
  ...
  bool canFinish(int numCourses, std::vector<std::vector<int>>& prerequisites) {
    ...
    for (int course_number = 0; course_number < numCourses; ++course_number) {
      if (adj[course_number].size() == 0) {
        // std::cout << "no prerequisites for course " << course_number <<
        // std::endl;
        takeable_courses.insert(course_number);
      } else if (takeable_courses.find(course_number) ==
                 takeable_courses.end()) {
        std::stack<int> frontier;
        std::for_each(adj[course_number].begin(), adj[course_number].end(),
                      [&frontier](auto v) { frontier.push(v); });
        bool cyclic = false;
        ...
        if (!cyclic) {
          // std::cout << "no cycle found. adding..." <<
...
TEST(Test, Test10) {
  Solution s;
  std::vector<std::vector<int>> a = {{1, 0}, {2, 6}, {1, 7},
                                     {6, 4}, {7, 0}, {0, 5}};
