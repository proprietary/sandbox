...
auto solve_part_1(std::vector<Block> expanded_line) -> uint64_t {
  ...
  size_t l = 0;
  while (l < r) {
    while (l < expanded_line.size() &&
           std::holds_alternative<File>(expanded_line[l])) {
      ++l;
    }
    while (r < static_cast<size_t>(-1) &&
           std::holds_alternative<Empty>(expanded_line[r])) {
      --r;
    }
    if (l >= r) {
      break;
  ...
  for (size_t i = 0; i < expanded_line.size() &&
                     !std::holds_alternative<Empty>(expanded_line[i]);
       ++i) {
